...
`off` 用于测纯图传和无线延时；`mlp` 是低延时 baseline；`tinycls` 是 6 类海面目标 Tiny CNN 分类模型；`coco` 是 COCO YOLO11n 320 参考检测模型；`yolo11` 和 `yolo26` 是当前已经完成训练、量化、嵌入和板端验证的 Coke/Sprite YOLO 模型。

轻量化路线已接入 `tinycls`：不再输出检测框，改为 Tiny CNN 分类输出 `label/score/top_k/inference_ms/analysis_ms`。任务定义、类别表和验收指标见 [tiny_cnn_classifier_cn.md](tiny_cnn_classifier_cn.md)，板端类别契约见 `main/tiny_cls_task.h`，ESP-DL INT8 模型为 `models/tiny_cls_96_6cls_s8_p4.espdl`。
