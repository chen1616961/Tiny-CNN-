...
#if CONFIG_IDF_TARGET_ESP32P4 && !defined(CONFIG_ESP_HOSTED_CP_TARGET_ESP32C6) && \
    !defined(CONFIG_SLAVE_IDF_TARGET_ESP32C6)
/* ESP-IDF 6.0 can leave the Wi-Fi Remote slave target as "invalid" after
 * regeneration. The ESP32-P4 Function EV Board uses the onboard ESP32-C6, so
 * keep the local build aligned with the board default.
 */
...
#define TINYCLS_MODEL_NAME "tiny-cnn-cls-" APP_STR(CONFIG_APP_TINY_CLS_INPUT_SIZE) "-6cls-p4"
#define YOLO26_INPUT_SIZE 416U
#define YOLO11_INPUT_SIZE 416U
#define COCO_INPUT_SIZE 320U
#define TINYCLS_INPUT_SIZE CONFIG_APP_TINY_CLS_INPUT_SIZE
...
static esp_err_t recognition_get_handler(httpd_req_t *req)
{
    ...
    if (strcmp(method_text, "off") == 0) {
        ...
        if (!tiny_cls_espdl_available()) {
            httpd_resp_set_status(req, "503 Service Unavailable");
            return httpd_resp_sendstr(req, "tinycls board backend is unavailable; check CONFIG_APP_TINY_CLS_MODEL_FILE");
        }
